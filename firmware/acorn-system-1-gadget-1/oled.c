...
const unsigned char font_5x7_letters[] = {
					  ...
					  0x44, 0x4C, 0x54, 0x64, 0x44, // z    7A
					  0x00, 0x41, 0x36, 0x08, 0x00, // {    7B
					  0x00, 0x00, 0x7F, 0x00, 0x00, // |    7C
					  0x00, 0x08, 0x36, 0x41, 0x00, // }    7D
					  0x08, 0x1C, 0x2A, 0x08, 0x08, // ->   7E
					  0x7F, 0x7F, 0x7F, 0x7F, 0x7F, //      7F   BLACK RECTANGLE
};
...
void oled_send_char(I2C_SLAVE_DESC *slave, int ch, int base)
{
#if !INVERT_DISP  
  oled_send_cmd(slave, 5, font_5x7_letters+('0'-' ')*5+ch*5, I2C_DATA, I2C_NO_REPEAT);
#else
  for(int i=4; i>=0; i--)
    {
      oled_send_cmd(slave, 1, font_5x7_letters+(ch-base)*5+i, I2C_DATA, I2C_NO_REPEAT);

      //oled_send_cmd(slave, 1, i+font_5x7_letters+('0'-' ')*5+ch*5, I2C_DATA, I2C_NO_REPEAT);
    }
#endif
}
      ...
      if( *string == '&' )
        {
          ...
          oled_set_xy(slave, last_set_x-2-1*FONT_WIDTH, last_set_y);
        }

      oled_send_char(slave, *string++, ' ');
